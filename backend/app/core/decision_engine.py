...
# Decision matrix calibrated for West African waste streams.
# The thresholds below are conservative and are used as explicit decision rules,
# not as a replacement for the technical/regulatory filters already in place.
FAMILY_DECISION_MATRIX = {
    "abattoir": {
        "methanisation_biogaz": {
            ...
            "min_dbo": 1200.0,
            "min_dco": 2500.0,
            "max_contamination": 35.0,
            "conditions": ["tri initial", "hygienisation des intrants", "drainage si humidite excessive"],
            "note": "Flux d'abattoir humide: digestion anaerobie prioritaire en contexte CEDEAO.",
        },
        "compostage": {
            "bonus": 8.0,
            "min_humidity": 45.0,
            "max_humidity": 75.0,
            "max_contamination": 30.0,
            "note": "Compostage possible seulement apres sanitisation et maitrise des odeurs.",
        },
        "reemploi": {"block_always": True, "block_reason": "Flux d'abattoir incompatible avec le reemploi direct."},
        "recyclage_matiere": {"block_always": True, "block_reason": "Flux d'abattoir incompatible avec le recyclage matiere direct."},
    },
    "organique": {
        "methanisation_biogaz": {
            "bonus": 18.0,
            ...
            "min_dco": 2500.0,
            "max_contamination": 35.0,
            "note": "Flux organique humide: la DBO/DCO elevee et l'humidite orientent vers le biogaz.",
        },
        "compostage": {
            "bonus": 12.0,
            "min_humidity": 40.0,
            ...
            "max_contamination": 35.0,
            "max_dco_dbo": 4.5,
            "note": "Humidite intermediaire: compostage pertinent si le lot reste biologiquement stabilisable.",
        },
        "biochar": {
            "bonus": 4.0,
            "min_lignine": 20.0,
            "max_humidity": 25.0,
            "min_pci": 12.0,
            "note": "Biochar reserve aux sous-fractions seches et plus lignifiees.",
        },
        "reemploi": {"block_always": True, "block_reason": "Flux organique humide incompatible avec le reemploi direct."},
        "recyclage_matiere": {"block_always": True, "block_reason": "Flux organique humide incompatible avec le recyclage matiere direct."},
    },
    "biomasse_lignocellulosique": {
        "biochar": {
            ...
            "max_humidity": 25.0,
            "min_pci": 12.0,
            "note": "Biomasse seche lignifiee: biochar et chaleur utile sont les voies de reference.",
        },
        "valorisation_energetique": {
            "bonus": 14.0,
            "min_pci": 14.0,
            "max_humidity": 30.0,
            "note": "PCI eleve et faible humidite: voie energetique robuste.",
        },
        "methanisation_biogaz": {"penalty": -18.0, "note": "Biomasse lignifiee: methanisation moins robuste sans co-substrat et pretraitement."},
        "compostage": {"penalty": -14.0, "note": "Biomasse lignifiee: compostage possible mais plus lent et plus exigeant."},
    },
    "plastique": {
        "recyclage_matiere": {
            "bonus": 22.0,
            ...
            "max_humidity": 40.0,
            "requires_chlore_absent": True,
            "note": "Plastique propre et sec: recyclage mecanique prioritaire.",
            "conditions": ["tri fin", "lavage si necessaire"],
        },
        "reemploi": {
            "bonus": 10.0,
            "max_contamination": 15.0,
            "max_humidity": 35.0,
            "note": "Reemploi reserve aux plastiques propres, homognes et peu dangereux.",
        },
        "pyrolyse_gazification": {
            "bonus": 12.0,
            "min_pci": 15.0,
            "max_humidity": 35.0,
            "requires_chlore_absent": True,
            "note": "Plastique sec et riche en PCI: stabilisation thermique possible si le chlore est maitrise.",
        },
        "co_incineration_cimenterie": {
            ...
            "max_humidity": 40.0,
            "requires_chlore_absent": True,
            "note": "Co-incineration possible pour les flux plastiques energetiquement valorisables.",
        },
        "methanisation_biogaz": {"block_always": True, "block_reason": "Flux plastique incompatible avec la methanisation directe."},
        "compostage": {"block_always": True, "block_reason": "Flux plastique incompatible avec le compostage direct."},
    },
    "textile": {
        "reemploi": {
            "bonus": 20.0,
            "max_contamination": 15.0,
            "max_humidity": 35.0,
            "requires_reusable_textile": True,
            "note": "Textile propre et reusable: reemploi prioritaire.",
        },
        "recyclage_matiere": {
            "bonus": 14.0,
            "max_contamination": 20.0,
            "max_humidity": 35.0,
            "note": "Textile propre: recyclage matiere ou effilochage restent pertinents.",
        },
        "methanisation_biogaz": {"penalty": -10.0, "note": "Textile: methanisation seulement si la fraction organique domine."},
    },
    "chimique": {
        "neutralisation_chimique": {
            "bonus": 30.0,
            "requires_chlore_or_hazard": True,
            "note": "Flux chimique ou reactif: stabilisation/neutralisation prioritaire avant toute autre voie.",
        },
    ...
    "metal": {
        "recyclage_matiere": {
            ...
            "max_humidity": 60.0,
            "note": "Flux metallique: recyclage matiere et refonte prioritaire.",
        },
        "vente": {
            "bonus": 12.0,
            "requires_metals": True,
            "note": "Vente certifiee pertinente si la filiere locale de reprise est disponible.",
        },
        "methanisation_biogaz": {"block_always": True, "block_reason": "Flux metallique incompatible avec la methanisation."},
        "compostage": {"block_always": True, "block_reason": "Flux metallique incompatible avec le compostage."},
        "reemploi": {"block_always": True, "block_reason": "Flux metallique incompatible avec le reemploi direct."},
    },
    "electronique": {
        "neutralisation_chimique": {
            "bonus": 24.0,
            "requires_hazard_or_metals": True,
            "note": "E-dechet: depollution/stabilisation avant orientation finale.",
        },
        "elimination_securisee": {
            "bonus": 18.0,
            "requires_hazard_or_metals": True,
            "note": "E-dechet complexe: securisation prioritaire si la recuperation n'est pas sure.",
        },
        "reemploi": {"block_always": True, "block_reason": "Flux electronique incompatible avec le reemploi direct sans controle specialise."},
        "recyclage_matiere": {"block_always": True, "block_reason": "Flux electronique incompatible avec le recyclage matiere standard."},
        "methanisation_biogaz": {"block_always": True, "block_reason": "Flux electronique incompatible avec la methanisation."},
        "compostage": {"block_always": True, "block_reason": "Flux electronique incompatible avec le compostage."},
    },
    "default": {},
}
...
def analyser_dechet(waste: WasteInput) -> DecisionResult:
    ...
    exp_payload = {
        ...
        "confiance": confiance,
        "filiere": chosen["filiere"],
        "hierarchie": chosen["hierarchy"],
        "conditions": chosen.get("conditions", []),
    }
    if llm_text:
        exp_payload["llm"] = llm_text
...
def _decision_family_key(w: WasteInput, wt: WasteType) -> str:
    if w.categorie == WasteCategory.ELECTRONIC:
        return "electronique"
    if w.categorie == WasteCategory.METAL:
        return "metal"
    if w.categorie == WasteCategory.CHEMICAL or _is_paint_or_coating_waste(w):
        return "chimique"
    if _is_abattoir_waste(w):
        return "abattoir"
    if wt == WasteType.BIOMASSE_LIGNOCELLULOSIQUE:
        return "biomasse_lignocellulosique"
    if wt == WasteType.HUILE_USAGEE:
        return "huile_usagee"
    if wt == WasteType.TEXTILE:
        return "textile"
    if wt == WasteType.PLASTIQUE or w.categorie == WasteCategory.PLASTIC or _is_probably_plastic_text(w):
        return "plastique"
    if _is_organic_waste_text(w) or w.categorie == WasteCategory.ORGANIC:
        return "organique"
    return _waste_family_label(w, wt)


def _family_rule_met(rule: dict[str, Any], w: WasteInput, profile: dict[str, Any]) -> bool:
    humidity = float(profile.get("humidity_pct") or 0.0)
    pci = float(profile.get("pci_mj_kg") or 0.0)
    lignine = float(profile.get("lignine_pct") or 0.0)
    dbo = float(profile.get("dbo_mg_l") or 0.0)
    dco = float(profile.get("dco_mg_l") or 0.0)
    contamination = float(profile.get("contamination_pct") or 0.0)
    dco_dbo = (dco / dbo) if dbo > 0 else None
    chlorine_risk = bool(profile.get("chlorine_risk") or w.presence_chlore or _is_pvc(w))
    metals = bool(w.contient_metaux or w.presence_metaux_lourds)
    danger = str(profile.get("danger") or "faible")

    checks: list[bool] = []
    if rule.get("min_humidity") is not None:
        checks.append(humidity >= float(rule["min_humidity"]))
    if rule.get("max_humidity") is not None:
        checks.append(humidity <= float(rule["max_humidity"]))
    if rule.get("min_pci") is not None:
        checks.append(pci >= float(rule["min_pci"]))
    if rule.get("min_lignine") is not None:
        checks.append(lignine >= float(rule["min_lignine"]))
    if rule.get("min_dbo") is not None:
        checks.append(dbo >= float(rule["min_dbo"]))
    if rule.get("min_dco") is not None:
        checks.append(dco >= float(rule["min_dco"]))
    if rule.get("max_contamination") is not None:
        checks.append(contamination <= float(rule["max_contamination"]))
    if rule.get("max_dco_dbo") is not None and dco_dbo is not None:
        checks.append(dco_dbo <= float(rule["max_dco_dbo"]))
    if rule.get("requires_chlore_absent"):
        checks.append(not chlorine_risk)
    if rule.get("requires_metals"):
        checks.append(metals)
    if rule.get("requires_reusable_textile"):
        checks.append(_textile_reusable(w))
    if rule.get("requires_chlore_or_hazard"):
        checks.append(chlorine_risk or danger in {"eleve", "critique"} or w.categorie in {WasteCategory.CHEMICAL, WasteCategory.ELECTRONIC})
    return all(checks) if checks else True


def _family_matrix_summary(w: WasteInput, wt: WasteType, profile: dict[str, Any]) -> str:
    family = _decision_family_key(w, wt)
    rules = FAMILY_DECISION_MATRIX.get(family, {})
    if not rules:
        return f"Matrice famille {family}: aucune regle specifique additionnelle."

    items: list[str] = []
    for route, rule in rules.items():
        if not isinstance(rule, dict) or rule.get("block_always"):
            continue
        parts: list[str] = []
        if rule.get("min_humidity") is not None:
            parts.append(f"hum >= {float(rule['min_humidity']):.0f}%")
        if rule.get("max_humidity") is not None:
            parts.append(f"hum <= {float(rule['max_humidity']):.0f}%")
        if rule.get("min_pci") is not None:
            parts.append(f"PCI >= {float(rule['min_pci']):.0f}")
        if rule.get("min_lignine") is not None:
            parts.append(f"lignine >= {float(rule['min_lignine']):.0f}%")
        if rule.get("min_dbo") is not None:
            parts.append(f"DBO >= {float(rule['min_dbo']):.0f}")
        if rule.get("min_dco") is not None:
            parts.append(f"DCO >= {float(rule['min_dco']):.0f}")
        if rule.get("max_contamination") is not None:
            parts.append(f"contamination <= {float(rule['max_contamination']):.0f}%")
        if rule.get("requires_chlore_absent"):
            parts.append("chlore absent")
        if rule.get("requires_metals"):
            parts.append("metaux recuperables")
        if rule.get("requires_reusable_textile"):
            parts.append("textile reutilisable")
        if parts:
            items.append(f"{route}: {'; '.join(parts)}")
    if not items:
        return f"Matrice famille {family}: famille identifiee, sans seuil additionnel explicite."
    return f"Matrice famille {family}: " + " | ".join(items[:4])


def _apply_family_matrix_rules(
    w: WasteInput,
    wt: WasteType,
...
) -> tuple[float, list[str], list[str], bool, str | None]:
    family = _decision_family_key(w, wt)
    family_rules = FAMILY_DECISION_MATRIX.get(family, FAMILY_DECISION_MATRIX["default"])
    rule = family_rules.get(str(candidate.get("filiere") or "")) if isinstance(family_rules, dict) else None
    if not isinstance(rule, dict):
        return tech, conditions, notes, feasible, blocked_reason

    matched = _family_rule_met(rule, w, profile)
    if rule.get("block_always"):
        tech -= 55.0
        feasible = False
        blocked_reason = str(rule.get("block_reason") or blocked_reason or "Matrice de decision: voie ecartee pour cette famille.")
        notes.append(blocked_reason)
        return tech, conditions, notes, feasible, blocked_reason

    if matched:
        bonus = float(rule.get("bonus", 0.0) or 0.0)
        if bonus:
            tech += bonus
        extra_conditions = [str(x) for x in (rule.get("conditions") or []) if str(x).strip()]
        if extra_conditions:
        ...
        if penalty:
            tech += penalty
        if rule.get("penalty_note"):
            notes.append(str(rule.get("penalty_note")))

    return tech, conditions, notes, feasible, blocked_reason

def _family_decision_table(w: WasteInput, wt: WasteType, profile: dict[str, Any], evald: list[dict[str, Any]]) -> list[dict[str, Any]]:
    family = _decision_family_key(w, wt)
    family_rules = FAMILY_DECISION_MATRIX.get(family, FAMILY_DECISION_MATRIX["default"])
    route_lookup = {str(item.get("filiere") or ""): item for item in evald}
    rows: list[dict[str, Any]] = []

    def _thresholds(rule: dict[str, Any]) -> dict[str, Any]:
        out: dict[str, Any] = {}
        for key in [
            "min_humidity",
            "max_humidity",
            ...
            "max_contamination",
            "max_dco_dbo",
            "requires_chlore_absent",
            "requires_metals",
            "requires_reusable_textile",
            "requires_chlore_or_hazard",
        ]:
            if rule.get(key) is not None:
                out[key] = rule.get(key)
        return out

    for filiere, rule in family_rules.items() if isinstance(family_rules, dict) else []:
        if not isinstance(rule, dict):
            continue
        route = route_lookup.get(str(filiere) or "", {})
        matched = _family_rule_met(rule, w, profile)
        feasible = bool(route.get("feasible", True))
        if rule.get("block_always"):
            matched = False
            feasible = False
        ...
        rows.append({
            ...
            "filiere": filiere,
            "solution": filiere,
            "score_technique": round(float(route.get("technical_score", 0.0) or 0.0), 2),
            "score_global": round(float(route.get("global_score", 0.0) or 0.0), 2),
            "conforme": matched,
            "feasible": feasible,
            "statut": "Bloquee" if not feasible else ("Conforme" if matched else "Hors seuil"),
            "ajustement": round(bonus + penalty, 2),
            "seuils": _thresholds(rule),
            "conditions": list(dict.fromkeys([*(route.get("conditions") or []), *(rule.get("conditions") or [])])),
            "justification": str(rule.get("note") or route.get("technical_reason") or route.get("blocked_reason") or ""),
        })

    if rows:
        return sorted(rows, key=lambda item: (not bool(item.get("feasible", True)), float(item.get("score_global", 0.0))), reverse=True)

    fallback = sorted(evald, key=lambda item: float(item.get("global_score", 0.0)), reverse=True)[:5]
    return [
        {
            "famille": family,
            "filiere": str(item.get("filiere") or ""),
            "solution": str(item.get("filiere") or item.get("solution") or ""),
            "score_technique": round(float(item.get("technical_score", 0.0) or 0.0), 2),
            "score_global": round(float(item.get("global_score", 0.0) or 0.0), 2),
            "conforme": bool(item.get("feasible", True)),
            "feasible": bool(item.get("feasible", True)),
            "statut": str(item.get("status") or ("Recommandee" if item.get("feasible", True) else "Bloquee")),
            "ajustement": 0.0,
            "seuils": {},
            "conditions": list(item.get("conditions") or []),
            "justification": str(item.get("technical_reason") or item.get("blocked_reason") or ""),
        }
...
def _apply_scientific_route_bias(w: WasteInput, wt: WasteType, m: dict[str, float], candidate: dict[str, Any]) -> tuple[float, list[str], list[str], bool, str | None]:
    ...
    if filiere == "methanisation_biogaz":
        ...
        tech, conditions, notes, feasible, blocked_reason = _apply_family_matrix_rules(
            w,
            wt,
            profile,
            candidate,
...
def _select(
    ...
    if material.get("reliable") and non_elimination:
        contamination = float(waste.taux_contamination_pct or 0.0)
        humidity = float(material.get("humidity_pct") or 0.0)
        ...
        hierarchy_best = next(
            (
                sorted(
                    [x for x in non_elimination if x["hierarchy"] == g],
                    key=lambda z: (rank(z), float(z.get("technical_score", 0.0))),
                    reverse=True,
                )[0]
                for g in HIERARCHY
                if any(x["hierarchy"] == g for x in non_elimination)
            ),
            None,
        )

        if hierarchy_best is not None:
            physico_margin = rank(best_overall) - rank(hierarchy_best)
            if best_overall.get("filiere") != hierarchy_best.get("filiere") and physico_margin >= 3.0:
                reasons.append("Score hybride physico-chimique superieur a la priorite hierarchique.")
                return best_overall, reasons

            if best_overall.get("filiere") == hierarchy_best.get("filiere"):
                reasons.append("Arbitrage physico-chimique et hierarchie convergents sur la meme filiere.")
                return best_overall, reasons

            chosen = hierarchy_best
            if chosen["hierarchy"] == "reemploi":
                reasons.append("Hierarchie appliquee: reemploi retenu en priorite (sobriete matiere/energie).")
            elif chosen["hierarchy"] == "matiere":
                reasons.append("Hierarchie appliquee: matiere retenue car faisable.")
            elif chosen["hierarchy"] == "energie":
                reasons.append("Energie retenue car matiere non faisable ou moins robuste.")
            else:
                reasons.append("Vente retenue en dernier recours seulement.")
            return chosen, reasons

        reasons.append("Arbitrage physico-chimique: meilleure voie faisable retenue faute de regle hierarchique applicable.")
        return best_overall, reasons

    chosen = best_overall
    reasons.append("Aucune voie de valorisation faisable: meilleure option conservee comme reference.")
    return chosen, reasons
